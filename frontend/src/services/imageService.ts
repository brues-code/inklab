 ...
 */
export const loadImage = async (imageType: ImageType, name?: string | null): Promise<string | null> => {
 ...
 */
export const loadNpcModel = async (displayId: number, creatureEntry = 0): Promise<string | null> => {
 ...
 */
export const loadNpcPortrait = async (displayId: number, creatureEntry = 0, generate = true): Promise<string | null> => {
 ...
 */
export const preloadImages = async (images: Array<{ type: ImageType; name: string; url?: string }>): Promise<void> => {
    await Promise.all(
        images.map(img => loadImage(img.type, img.name))
    );
};
