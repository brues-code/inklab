...
func (i *MySQLImporter) ImportAllFromMySQL() error {
	...
	// 2b. Creature Equipment (maps equipment_id -> 3 equipped item entries, used
	// to render held weapons on NPC models).
	if err := i.runImportIfEmpty("creature_equip_template", "entry,equipentry1,equipentry2,equipentry3"); err != nil {
		log.Printf("Warning: Failed to import creature_equip_template: %v", err)
	}

	// 2c. Trainer spell lists — reqskill/reqskillvalue is the authoritative skill
	// rank a recipe's learn-spell is taught at (e.g. Runed Golden Rod = Enchanting
	// 150), which the craft spell's own SkillLineAbility doesn't carry.
	trainerCols := "entry,spell,spellcost,reqskill,reqskillvalue,reqlevel"
	if err := i.runImportIfEmpty("npc_trainer", trainerCols); err != nil {
		log.Printf("Warning: Failed to import npc_trainer: %v", err)
	}
	if err := i.runImportIfEmpty("npc_trainer_template", trainerCols); err != nil {
		log.Printf("Warning: Failed to import npc_trainer_template: %v", err)
	}

	// collection_mount maps a mount item to the mount spell it grants, bridging
	// item -> mount spell (aura 78) -> creature display id for the model render.
	if err := i.runImportIfEmpty("collection_mount", "itemId,spellId"); err != nil {
		log.Printf("Warning: Failed to import collection_mount: %v", err)
	}

	// item_enchantment_template: which random suffixes ("of the Monkey") an
	// item's random_property can roll, with relative weights.
	if err := i.runImportIfEmpty("item_enchantment_template", "entry,ench,chance"); err != nil {
		log.Printf("Warning: Failed to import item_enchantment_template: %v", err)
	}
